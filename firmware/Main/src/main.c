...
#include "Drivers/Lcd/lcd_interface.h"
...
void main_initRobot(void) {
  ...
  lcd_init(LCD_RST_GPIO_Port, LCD_RST_Pin, LCD_EN_GPIO_Port, LCD_EN_Pin,
           ...
           LCD_D6_GPIO_Port, LCD_D6_Pin, LCD_D7_GPIO_Port, LCD_D7_Pin, 2, 16);

  txt_init(&main_robotInstance.usbUart, USB_UART, USB_UART_IR, USB_UART_DMA_IR,
           500, 500, "\r");
  txt_init(&main_robotInstance.espUart, ESP_UART, ESP_UART_IR, ESP_UART_DMA_IR,
           500, 500, "\r\x01\x02\x03");

#if US_SENSOR
  us_init(&main_robotInstance.us, US_TRIG_GPIO_Port, US_TRIG_Pin,
          RANGE_COLOR_ESP_TIMER, US_TIMER_FREQUENCY_HZ, RANGE_COLOR_ESP_TIMER,
          US_TIMER_FREQUENCY_HZ);
  ...
  clr_init(&main_robotInstance.colorSensor, COLOR_S0_GPIO_Port, COLOR_S0_Pin,
           COLOR_S1_GPIO_Port, COLOR_S1_Pin, COLOR_S2_GPIO_Port, COLOR_S2_Pin,
           COLOR_S3_GPIO_Port, COLOR_S3_Pin, 16);
  ...
  enc_init(&main_robotInstance.rightEncoder, ENC1_A_GPIO_Port, ENC1_A_Pin,
           ENC1_B_GPIO_Port, ENC1_B_Pin, MOTOR1_ENCODER_RESOLUTION,
           MOTOR1_REVERSED, LCD_IMU_ENCODER_TIMER, MOTOR_ENCODER_TIMER_FREQ,
           MOTOR_ENCODER_MAX_SPEED_CPS);
  enc_init(&main_robotInstance.leftEncoder, ENC2_A_GPIO_Port, ENC2_A_Pin,
           ENC2_B_GPIO_Port, ENC2_B_Pin, MOTOR2_ENCODER_RESOLUTION,
           MOTOR2_REVERSED, LCD_IMU_ENCODER_TIMER, MOTOR_ENCODER_TIMER_FREQ,
           MOTOR_ENCODER_MAX_SPEED_CPS);

  {
    pwm_Pwm pwm1;
    pwm_init(&pwm1, MOTOR12_PWM1_TIMER, MOTOR1_PWM1_TIMER_CHANNEL,
             MOTOR1_PWM1_TIMER_PERIOD, MOTOR1_PWM1_OUTPUT_TYPE);
    pwm_Pwm pwm2;
    pwm_init(&pwm2, MOTOR12_PWM2_TIMER, MOTOR1_PWM2_TIMER_CHANNEL,
             MOTOR1_PWM2_TIMER_PERIOD, MOTOR1_PWM2_OUTPUT_TYPE);
    drv_motorInit(&main_robotInstance.rightMotor, pwm1, pwm2, MOTOR1_REVERSED);
    drv_speedControlInit(&main_robotInstance.rightSpeedCtrl,
                         &main_robotInstance.rightMotor,
                         &main_robotInstance.rightEncoder);
  }
  {
    pwm_Pwm pwm1;
    pwm_init(&pwm1, MOTOR12_PWM1_TIMER, MOTOR2_PWM1_TIMER_CHANNEL,
             MOTOR2_PWM1_TIMER_PERIOD, MOTOR2_PWM1_OUTPUT_TYPE);
    pwm_Pwm pwm2;
    pwm_init(&pwm2, MOTOR12_PWM2_TIMER, MOTOR2_PWM2_TIMER_CHANNEL,
             MOTOR2_PWM2_TIMER_PERIOD, MOTOR2_PWM2_OUTPUT_TYPE);
    drv_motorInit(&main_robotInstance.leftMotor, pwm1, pwm2, MOTOR2_REVERSED);
    drv_speedControlInit(&main_robotInstance.leftSpeedCtrl,
                         &main_robotInstance.leftMotor,
                         &main_robotInstance.leftEncoder);
  }
  ...
  {
    pwm_Pwm pwm;
    pwm_init(&pwm, SERVO_DELAYUS_TIMER, SERVO_CHANNEL, SERVO_PWM_PERIOD,
             PwmOutput_P);
    srv_init(&main_robotInstance.servo, pwm, SERVO_START_POS, SERVO_END_POS,
             SERVO_INIT_POS);
  }
