...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim) {
  ...
  if (htim == LCD_IMU_ENCODER_TIMER) {
    ...
    {
      orCnt = 0;
      mpu_updateOrientation(&main_robotInstance.imu,
                            &main_robotInstance.orientation, 0.008125f);
    }
  }
  ...
  if (htim == VBAT_ADC_TIMER) {
    ...
    if (adcCnt == BATTERY_INDICATOR_PERIOD) {
      adcCnt = 0;
      lcd_displayStatus(BATTERY_INDICATOR_ROW, BATTERY_INDICATOR_COL,
                        batteryVoltage, espState);
    }
  ...
  if (htim == RANGE_COLOR_ESP_TIMER) {
    switch (htim->Channel) {
      ...
      case US_RISING_ACTIVE_CHANNEL:
        us_handlerRisingCapture(
            &main_robotInstance.us,
            HAL_TIM_ReadCapturedValue(htim, US_RISING_CHANNEL));
        break;
      case US_FALLING_ACTIVE_CHANNEL:
        us_handlerFallingCapture(
            &main_robotInstance.us,
            HAL_TIM_ReadCapturedValue(htim, US_FALLING_CHANNEL));
        break;
#elif IR_SENSOR
      case IR_RISING_ACTIVE_CHANNEL:
        ir_handlerRisingCapture(
            &main_robotInstance.ir,
            HAL_TIM_ReadCapturedValue(htim, IR_RISING_CHANNEL));
        break;
      case IR_FALLING_ACTIVE_CHANNEL:
        ir_handlerFallingCapture(
            &main_robotInstance.ir,
            HAL_TIM_ReadCapturedValue(htim, IR_FALLING_CHANNEL));
        break;
#endif
      case COLOR_ACTIVE_CHANNEL:
        clr_captureHandler(&main_robotInstance.colorSensor,
                           HAL_TIM_ReadCapturedValue(htim, COLOR_CHANNEL));
        break;
      default:
        break;
    }
...
#if US_SENSOR
  if (htim == RANGE_COLOR_ESP_TIMER &&
      htim->Channel == US_ASYNC_ACTIVE_CHANNEL) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart) {
  txt_handleReceiveCplt(&main_robotInstance.usbUart, USB_UART,
                        main_robotInstance.initCplt);
  char c = txt_handleReceiveCplt(
      &main_robotInstance.espUart, ESP_UART,
      main_robotInstance.initCplt && main_robotInstance.espReady);
  if (c)
    espState = c;
}
