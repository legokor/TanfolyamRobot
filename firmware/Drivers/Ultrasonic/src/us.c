 ...
 *
 * The echo signal measurement is based on the timer input capture function.
 * STM32 timer channels don't support capturing on both rising and falling edges
 * of an input, so two timer channels must be used: one in direct mode and one
 * in indirect mode. In indirect mode the channel's edge detector is routed to
 * the neighboring channel's input, so only one external connection is
 * necessary.
 */
...
// Speed of sound in air is about 340 m/s, so the width of the echo signal is 29
// us/cm. The distance is measure in both directions, so we need to double that.
#define US_CM_DIVIDER 58

// How often the distance is measured in the background (1 overflow is roughly
// 33ms)
...
/**
 * Call this from a timer's capture interrupt on falling edges of the echo
 * signal
 * @param us
 * @param captureVal the captured value from the timer channel
 */
