...
void clr_captureHandler(clr_ColorSensor* cs, uint16_t captureVal) {
  ...
  if (cs->itCntr == cs->numOfAverages) {
    ...
    cs->measuredVal[cs->currentCaptureIndex] =
        cs->captureSum / cs->numOfAverages;
    cs->captureSum = 0;
...
void clr_init(clr_ColorSensor* cs,
              GPIO_TypeDef* csS0Port,
              uint16_t csS0Pin,
              GPIO_TypeDef* csS1Port,
              uint16_t csS1Pin,
              GPIO_TypeDef* csS2Port,
              uint16_t csS2Pin,
              GPIO_TypeDef* csS3Port,
              uint16_t csS3Pin,
 ...
 * @param capClear
 */
void clr_getPeriods(clr_ColorSensor* cs,
                    uint16_t* capRed,
                    uint16_t* capGreen,
                    uint16_t* capBlue,
                    uint16_t* capClear) {
