...
// If the count does not change for this many timer overflows, we can assume
// that the motor is stopped.
#define SPEED_TICK_TIMEOUT 500

typedef enum { nop, inc, dec } enc_CounterOperation;
 ...
 * @param reversed 0 will increase counter when A is leading, otherwise increase
 * counter when B is leading
...
void enc_init(enc_Encoder* encoder,
              ...
              uint8_t reversed,
              TIM_HandleTypeDef* intervalTimer,
...
/**
 * Call this from the GPIO interrupt handler when there's an interrupt on the A
 * terminal
 * @param encoder
 */
void enc_handlerA(enc_Encoder* encoder) {
  ...
  int32_t elapsed =
      overflowCount * encoder->timerPeriod + timerVal - encoder->lastTimerVal;
  ...
  if (risingEdgeA) {
    ...
    if ((encoder->resolution == EncoderResolution_2) ||
        (encoder->resolution == EncoderResolution_4)) {
...
/**
 * Call this from the GPIO interrupt handler when there's an interrupt on the B
 * terminal
 * @param encoder
 */
