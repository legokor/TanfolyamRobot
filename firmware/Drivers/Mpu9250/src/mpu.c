...
#ifdef ASYNC_IMU
/**
 * @brief This function is only present when using the async version of the
 * class. It needs to be called whenever the HAL_I2C_MemRxCpltCallback function
 * is called.
 *
 */
...
/**
 * @brief This function is only present when using the async version of the
 * class. Call it whenever the I2C period timer overflows. It is advised to call
 * this function at least as frequently as the highest sampling frequency set
 * for any sensor, but not too frequently - the transmission of ~24 bytes must
...
/**
 * @brief This function is only present when using the async version of the
 * class. You can check whether new data is available to read using it.
 *
 ...
 *
 * @param hi2c - the I2C HAL object pointer. The global interrupts must be
 * enabled in the CubeMX.
 * @param imuAddress - the IMU IC I2C address (in reality this is the MPU9250
...
/**
 * @brief Sets the IMU to the default settings, which will suite most
 * applications:
 * 	- The magnetometer is set to 16 bit resolution with a 100Hz sample rate
 * 	- The accelerometer is set to +-4g range with a 200Hz sample rate and
 * a 44.8Hz DLPF
 * 	- The gyro and thermometer are set to 2000DPS range (in case of the
 ...
 * The divider is only active when DLPF is enabled and set to 1-6 (both in case
 * of the gyro/temp and the accelerometer).
 *
...
/**
 * @brief Enable DLPF for the accelerometer (set fchoice_b to 0 -> fchoice to
 * 1).
 *
...
/**
 * @brief Enable DLPF for the gyro and temp sensors (set fchoice_b's to 0 ->
 * fchoice's to 1).
 *
...
/**
 * @brief Details about the values:
