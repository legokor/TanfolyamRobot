...
void mpu_updateOrientation(mpu_Mpu9250* imu, volatile mpu_Orientation* orientation, float dt)
{
    ...
    orientation->pitch -= orientation->roll * sin(gyro.z * 0.01745f * dt); // If the IMU has yawed transfer the roll
                                                                           // angle to the pitch angel.
    orientation->roll += orientation->pitch * sin(gyro.z * 0.01745f * dt); // If the IMU has yawed transfer the pitch
                                                                           // angle to the roll angel.

    // Accelerometer angle calculations
    float acc_total_vector =
        sqrt((acc.x * acc.x) + (acc.y * acc.y) + (acc.z * acc.z)); // Calculate the total accelerometer vector.
    ...
    orientation->pitch =
        orientation->pitch * 0.995 + angle_pitch_acc * 0.005;               // Correct the drift of the gyro pitch angle
                                                                            // with the accelerometer pitch angle.
    orientation->roll = orientation->roll * 0.995 + angle_roll_acc * 0.005; // Correct the drift of the gyro roll angle
