 ...
 *      Author: ksstms
 *
 * Most public functions of this library are non-blocking. They just put
 * commands in a buffer, which are sent via the handler function. The handler
 * function shall be called from a timer interrupt. Calls should occur at least
 * ~20 us apart, but the exact minimum might be display dependent.
 *
 ...
 */
void lcd_init(GPIO_TypeDef* lcdRsPort,
              uint16_t lcdRsPin,
              GPIO_TypeDef* lcdEnPort,
              uint16_t lcdEnPin,
              GPIO_TypeDef* lcdD4Port,
              uint16_t lcdD4Pin,
              GPIO_TypeDef* lcdD5Port,
              uint16_t lcdD5Pin,
              GPIO_TypeDef* lcdD6Port,
              uint16_t lcdD6Pin,
 ...
 * @note CGRAM is located at character address 0-7, and they are duplicated at
 * 8-15
 ...
 * @note \n and \r are the only special characters supported. Everything else
 * will be sent to the LCD as-is.
 *
...
/**
 * Clear the screen
 * This function is blocking for 5 ms so no other commands will be sent until
