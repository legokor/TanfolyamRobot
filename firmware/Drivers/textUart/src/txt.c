...
void txt_handleTransmitCplt(txt_Uart* uart, UART_HandleTypeDef* huart) {
  ...
  if (uart->startOfWriteData <= uart->endOfWriteData) {
    ...
    HAL_UART_Transmit_DMA(
        uart->huart,
        (uint8_t*)uart->writeCircularBuffer + uart->startOfWriteData,
        charCount);
    uart->startOfWriteData = -1;
  } else {
...
void txt_transmit(txt_Uart* uart, const char* str, const uint32_t size) {
  ...
  if (spaceTillBufferEnd >= size) {
    memcpy((void*)uart->writeCircularBuffer + uart->endOfWriteData + 1,
           (const void*)str, size);
    ...
    if (uart->startOfWriteData == -1) {
      if (uart->transmissionInProgress) {
        ...
        HAL_UART_Transmit_DMA(
            uart->huart,
            (uint8_t*)uart->writeCircularBuffer + uart->endOfWriteData + 1,
            size);
    ...
    if (spaceTillBufferEnd > 0)
      memcpy((void*)uart->writeCircularBuffer + uart->endOfWriteData + 1,
             (const void*)str, spaceTillBufferEnd);
    memcpy((void*)uart->writeCircularBuffer,
           (const void*)str + spaceTillBufferEnd, size - spaceTillBufferEnd);
    HAL_NVIC_DisableIRQ(uart->uartIr);
    HAL_NVIC_DisableIRQ(uart->sendDMAIr);
    if (uart->startOfWriteData == -1) {
      if (spaceTillBufferEnd == 0) {
        if (uart->transmissionInProgress) {
          ...
          HAL_UART_Transmit_DMA(uart->huart,
                                (uint8_t*)uart->writeCircularBuffer, size);
        }
...
char txt_handleReceiveCplt(txt_Uart* uart,
  ...
  while (*ptr) {
    if (c == *ptr) {
      HAL_UART_Receive_IT(
          uart->huart, (uint8_t*)uart->readCircularBuffer + uart->readPtr, 1);
      return c;
    }
    ptr++;
  }
  ...
  HAL_UART_Receive_IT(uart->huart,
                      (uint8_t*)uart->readCircularBuffer + uart->readPtr, 1);
  return 0;
}
...
uint8_t txt_receive(txt_Uart* uart, char* data) {
  ...
  if (startOfData > newLine) {
    ...
    memcpy(data, (const void*)uart->readCircularBuffer + startOfData,
           newLine - startOfData);
