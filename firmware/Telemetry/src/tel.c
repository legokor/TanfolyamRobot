...
static uint32_t tel_packetSize = 0;

static_assert(sizeof(tel_OutputData) + 2 <= TEL_MAX_DATA_SIZE,
              "tel_OutputData size is too big for the data buffer");

uint8_t tel_initEsp() {
  uint32_t size = snprintf(tel_dataOut, TEL_MAX_DATA_SIZE, "C\t%s\t%s\t%s",
...
uint8_t tel_convertDataToPacket(uint32_t size) {
